...
class AppController extends AbstractController
{
    ...
    #[Route('/user/{id}/following', name: 'app_user_following', requirements: ['id' => '\d+'])]
    public function userFollowing(int $id, EntityManagerInterface $em): Response
    {
        $targetUser = $em->getRepository(\App\Entity\User::class)->find($id);
        if (!$targetUser) {
            throw $this->createNotFoundException('User not found');
        ...
        $followers = $em->getRepository(\App\Entity\Follower::class)->findBy(['user' => $targetUser]);
        $authors = [];
    ...
    #[Route('/user/{id}/likes', name: 'app_user_likes', requirements: ['id' => '\d+'])]
    public function userLikes(int $id, EntityManagerInterface $em): Response
    {
        $targetUser = $em->getRepository(\App\Entity\User::class)->find($id);
        if (!$targetUser) {
            throw $this->createNotFoundException('User not found');
        ...
        $interactions = $em->getRepository(\App\Entity\ContentInteraction::class)->findBy(
            ['user' => $targetUser, 'interactionType' => 'like'],
            ['createdAt' => 'DESC']
        );
        $feed = [];
